        ...
        btnSetFilter.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (etLen.getText().toString() == null || etLen.getText().toString().isEmpty()) {
                    UIHelper.ToastMessage(mContext, "Độ dài dữ liệu không thể trống");
                    return;
                }
                if (etPtr.getText().toString() == null || etPtr.getText().toString().isEmpty()) {
                    UIHelper.ToastMessage(mContext, "Địa chỉ bắt đầu không thể trống");
                    return;
                }
                ...
                if (len > 0) {
                    String rex = "[\\da-fA-F]*"; //Khớp với biểu thức chính quy, dữ liệu ở định dạng thập lục phân
                    if (data == null || data.isEmpty() || !data.matches(rex)) {
                        UIHelper.ToastMessage(mContext, "Dữ liệu cần lọc phải là dạng số hệ cơ số mười lăm (thập lục phân)");
                        return;
                    }
